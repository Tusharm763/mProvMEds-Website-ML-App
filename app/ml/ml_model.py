...
from dot_env import DISEASE_TEST_DATA,DISEASE_TRAIN_DATA
...
for i in sorted(feature):
    j = str(i).replace("_", " ")
    symptom_set[i] = str(i).replace("_", " ").upper()
...
# Model Training
list_of_user_symptoms_2 = [0] * len(feature)

# Load training data
Training_Data = pd.read_csv(DISEASE_TRAIN_DATA)
Testing_Data = pd.read_csv(DISEASE_TEST_DATA)
...
for i in [Testing_Data, Training_Data]:
    i.replace({'prognosis': disease_mapping}, inplace=True)
...
# Model ensemble
clf0 = svm.SVC(kernel='linear', probability=True)
clf1 = KNeighborsClassifier(n_neighbors=4)
clf2 = RandomForestClassifier(n_estimators=100)
clf3 = VotingClassifier(estimators=[("SVM", clf0), ("KNN", clf1), ("RandomF", clf2)], voting='soft')
